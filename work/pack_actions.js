            ...
            if (dealmsgs.length == size) {
                                    ...
                                    if (processed.error != null) {
                                        ...
                                        for (let j in signTxs) {
                                            ...
                                            for (let k in failed_actions) {
                                                ...
                                                failedQueue.push(hexData, function (err) {
                                        ...
                                        console.log("One transaction including 50 actions is failed!")
                                    } else {
                ...
                createTxLocalByActionPool(tx => {
                    eos.pushTransaction(tx.transaction).then(ret => {
                        let actions_length = tx.transaction.transaction.actions.length;
                        console.log("Not full as a batch unit, size: " + actions_length);
                        ...
                        if (processed.error != null) {
                            ...
                            console.log("One transaction including " + actions_length + " actions is failed! ")
                        } else {
                            let actions = processed.action_traces;
                            for (let key in actions) {
                                resQueueProductor(actions[key].act.data.memo, "success", 200, resQueue);
